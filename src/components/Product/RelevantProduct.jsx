...
const RelevantProduct = ({ recommended }) => {
  ...
  return (
    <div>
      <div className="mx-10">
        <div>
          <h1 class="text-3xl sm:text-xl font-bold text-slate-900 mb-6 sm:mb-8">
            Relevant products
          </h1>
        </div>
        <div class=" lg:max-w-6xl md:max-w-4xl">
            ...
            {recommended.map((product) => (
              <div class="bg-white flex flex-col rounded overflow-hidden shadow-md hover:scale-[1.01] transition-all relative">
                <Link to={`/products/${product.slug}` } onClick={handleClick}>
                  ...
                  <div class="p-4">
                    <h5 class="text-sm sm:text-base font-semibold text-slate-900 line-clamp-2 min-h-[3rem]">
                      {product.name}
                    </h5>
